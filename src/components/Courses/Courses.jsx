    ...
    return (
        <VStack className='course' alignItems={['center','flex-start']}>
            ...
            <Heading textAlign={['center','left']} maxW={'200px'} fontFamily={'sans-serif'} 
                noOfLines={3} size={'sm'} children={title} /> 
            ...
            <Heading textAlign={'flex-end'}  textTransform={'uppercase'} 
                size={'xs'} children={`Lecture - ${lectureCount}`}/>
            <Heading size={'xs'} textTransform={'lowercase'} children={`views - ${views}`}/>
            <Stack direction={['column','row']} alignItems={'center'}>
                <Link to={`/course/${id}`}>
                    <Button colorScheme='yellow'> Watch Now</Button>
                </Link>
                <Button isLoading={loading} variant={'ghost'} colorScheme='yellow' onClick={()=>addToPlaylistHandler(id)}> Add to Playist</Button>
            </Stack>
        </VStack>
    )
}
const Courses = () => {
  ...
  return <Container minH={'95vh'} maxW='container.lg' paddingY={'8'}>
    ...
    <Stack direction={['column','row']} justifyContent={['flex-start','space-evenly']} 
        flexWrap={'wrap'} alignItems={['center','flex-start']}>
